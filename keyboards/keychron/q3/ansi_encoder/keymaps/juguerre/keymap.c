...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [WIN_BASE] = LAYOUT_tkl_f13_ansi(
        ...
        KC_GRV,   KC_1,     KC_2,     KC_3,     KC_4,     KC_5,     KC_6,     KC_7,     KC_8,     KC_9,     KC_0,     KC_MINS,  KC_EQL,     KC_BSPC,  KC_INS,   KC_HOME,  KC_PGUP,
        KC_TAB,   KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,     KC_Y,     KC_U,     KC_I,     KC_O,     KC_P,     KC_LBRC,  KC_RBRC,    KC_BSLS,  KC_DEL,   KC_END,   KC_PGDN,
        KC_CAPS,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,  TD(TD_SCLN_COLN),KC_QUOT, KC_ENT,
        KC_LSFT,            KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,              KC_RSFT,            KC_UP,
        KC_LCTL,  KC_LWIN,  KC_LALT,                                LT(3,KC_SPC),                          KC_RALT,  QK_MACRO_0,QK_MACRO_1,KC_RCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT),
    ...
    [WIN_FN] = LAYOUT_tkl_f13_ansi(
        ...
        _______,            _______,  _______,  _______,  _______,  _______,  KC_BSPC,  KC_DEL,   _______,  _______,  _______,              _______,            KC_MS_U,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        switch (keycode) {
            case QK_MACRO_0:
                // {KC_LCTL,KC_LALT}{KC_LCTL,KC_LGUI,KC_LEFT}
                SEND_STRING(SS_DOWN(X_LCTL)SS_TAP(X_LALT)SS_UP(X_LCTL)SS_DELAY(100)SS_DOWN(X_LCTL)SS_DOWN(X_LGUI)SS_TAP(X_LEFT)SS_UP(X_LCTL)SS_UP(X_LGUI)SS_TAP(X_BTN1));
                return false;
            case QK_MACRO_1:
                // {KC_LCTL,KC_LALT}{KC_LCTL,KC_LGUI,KC_RGHT}
                SEND_STRING(SS_DOWN(X_LCTL)SS_TAP(X_LALT)SS_UP(X_LCTL)SS_DELAY(100)SS_DOWN(X_LCTL)SS_DOWN(X_LGUI)SS_TAP(X_RGHT)SS_UP(X_LCTL)SS_UP(X_LGUI)SS_TAP(X_BTN1));
                return false;
            case QK_MACRO_2:
                // {KC_LCTL,KC_C}  COPY
